...
Description  : 多进程日志处理
FilePath     : /VehTypeCameraServer/trunk/common/multiprocess_log_handler.py
Reference: https://www.jianshu.com/p/d874a05edf19
"""
import datetime
import logging
import os
...
class MultiprocessLogHandler(logging.FileHandler):
    ...
    def __init__(self, filename, when='D', backupCount=0, encoding=None, delay=False):
        ...
        # 拼接文件路径 格式化字符串
        self.filefmt = os.path.join("log", "%s.%s" % (self.prefix, self.suffix))
        # 使用当前时间，格式化文件格式化字符串
